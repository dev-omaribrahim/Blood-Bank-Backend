...
class BaseDonation(models.Model):
    ...
    donation_type = models.CharField(
        max_length=255,
        choices=choices.DONATION_TYPE_CHOICES,
        null=True,
        blank=True,
    ...
    unit_serial_number = models.CharField(
        max_length=255, unique=True, null=True, blank=True, verbose_name="سيريال الوحدة"
    )
    ...
    analyse_status = models.CharField(
        ...
        null=True,
        blank=True,
        choices=choices.ANALYSE_STATUS,
        verbose_name="نتيجة تحليل الوحدة",
    )
    unit_notes = models.CharField(
        max_length=255,
        choices=choices.UNITE_NOTES_CHOICES,
        null=True,
        blank=True,
    ...
    unit_exchange_status = models.CharField(
        max_length=255,
        choices=choices.EXCHANGE_STATUS_CHOICES,
        null=True,
        blank=True,
    ...
    is_sold = models.BooleanField(
        null=True, blank=True, default=False, verbose_name="هل تم بيعه ؟"
    )
    is_separable = models.BooleanField(
        null=True, blank=True, default=False, verbose_name="هل قابلة للفصل ؟"
    )
    ...
    def save(self, *args, **kwargs):
        self.donation_expire_date = self.donation_create_date + timedelta(
            days=self.donation_expiration_scope
        )

        if (
            self.unit_type == choices.PLASMA
            or self.unit_type == choices.BLOOD_PLATELETS
            or self.unit_type == choices.CRYO
        ):
            self.blood_mapper()
...
class InsideDonation(BaseDonation):
    ...
    is_derivative = models.BooleanField(
        default=False, null=False, blank=False, verbose_name="هل هى مشتقة ؟"
    )
    ...
    def save(self, *args, **kwargs):
        if (
            (self.unit_type == choices.FULL_BLOOD or self.unit_type == choices.PLASMA)
            and not self.used_for_deriving
            and self.analyse_status == choices.FREE
        ):
...
class OutsideDonation(BaseDonation):
    ...
    def __str__(self):
        return "{} / {}".format(
            self.get_unit_type_display(), self.get_donation_type_display()
        )
...
class ReplaceDonation(BaseDonation):
    operation_type = models.CharField(
        max_length=255,
        choices=choices.OPERATION_TYPE,
        null=True,
        blank=False,
...
class DonorProfile(models.Model):
    full_name = models.CharField(
        max_length=255, null=False, blank=False, verbose_name="الاسم بالكامل"
    )
    age = models.DateField(null=False, blank=False, verbose_name="السن")
    phone_number = models.CharField(
        max_length=11, null=False, blank=False, verbose_name="رقم الموبايل"
    )
    address = models.CharField(
        max_length=255, null=False, blank=False, verbose_name="العنوان"
    )
    national_id = models.PositiveIntegerField(
        unique=True, null=False, blank=False, verbose_name="رقم البطاقة او الباسبور"
    )
    ...
    blood_type = models.CharField(
        ...
        null=False,
        blank=False,
        choices=choices.DONOR_BLOOD_TYPE_CHOICES,
        default=choices.A_POSITIVE,
        verbose_name="فصيلة الدم",
    )
    ...
    created_at = models.DateField(
        auto_now_add=True, verbose_name="وقت انشاء الملف", null=True, blank=False
    )
    ...
    def __str__(self):
        return "{} / {}".format(self.full_name, self.national_id)


class InstituteProfile(models.Model):
    full_name = models.CharField(
        max_length=255, null=False, blank=False, verbose_name="اسم المنشأة"
    )
    phone_number = models.CharField(
        max_length=11, null=False, blank=False, verbose_name="رقم الهاتف"
    )
    address = models.CharField(
        max_length=255, null=False, blank=False, verbose_name="العنوان"
    )
    created_at = models.DateField(
        auto_now_add=True, verbose_name="وقت انشاء الجهة", null=True, blank=False
    )
...
class DonationExpirationScope(models.Model):
    scope_label = models.CharField(
        max_length=100, null=True, blank=False, unique=True, verbose_name="تعريف المدة"
    )
