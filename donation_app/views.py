...
class SeparationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if {"parent_serial_number", "type_of_derivative_unit", "donation_expiration_scope"} <= request.data.keys():
            ...
            try:
                parent_obj = InsideDonation.objects.get(unit_serial_number=request_data["parent_serial_number"])
            ...
            if request_data["type_of_derivative_unit"] == choices.PLASMA and \
                    parent_obj.unit_type == choices.FULL_BLOOD and parent_obj.is_separable:
                ...
                new_child_obj = InsideDonation.objects.create(
                    ...
                    donor_profile=DonorProfile.objects.get(id=parent_obj.donor_profile.id),
                    is_derivative=True,
                )
            ...
            # and didn't separated before if so then create the unit
            elif request_data["type_of_derivative_unit"] == choices.BLOOD_PLATELETS and \
                    parent_obj.unit_type == choices.PLASMA and parent_obj.is_separable:
            ...
            # and didn't separated before if so then create the unit
            elif request_data["type_of_derivative_unit"] == choices.CRYO and \
                    parent_obj.unit_type == choices.PLASMA and parent_obj.is_separable:
...
class OldDonationCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if {"donation_type", "donator_profile_id", "donation_data"} <= request.data.keys():
            request_data = request.data

            if request_data["donation_type"] == choices.INSIDE_DONATION:

                try:
                    donor_profile = DonorProfile.objects.get(pk=request_data["donator_profile_id"])
                except DonorProfile.DoesNotExist:
                    return Response("Not Found !", status=status.HTTP_204_NO_CONTENT)

                donation_serializer = InsideDonationSerializer(data=request_data["donation_data"])
                ...
                try:
                    receipt = Receipt.objects.get(receipt_number=request_data["donator_profile_id"])
                except Receipt.DoesNotExist:
                    return Response("Object Not Found !", status=status.HTTP_204_NO_CONTENT)

                outside_donation_serializer = OutsideDonationSerializer(data=request_data["donation_data"])
                if outside_donation_serializer.is_valid():
                    outside_donation_serializer.save(receipt=receipt)
                    return Response(outside_donation_serializer.data, status=status.HTTP_200_OK)
                else:
                    return Response(outside_donation_serializer.errors, status=status.HTTP_400_BAD_REQUEST)

            elif request_data["donation_type"] == choices.REPLACE_DONATION:
                try:
                    institute = InstituteProfile.objects.get(id=request_data["donator_profile_id"])
                except InstituteProfile.DoesNotExist:
                    return Response("Not Found !", status=status.HTTP_204_NO_CONTENT)
                replace_donation_serializer = ReplaceDonationSerializer(data=request_data["donation_data"])
                if replace_donation_serializer.is_valid():
                    replace_donation_serializer.save(institute_profile=institute)
                    return Response(replace_donation_serializer.data, status=status.HTTP_200_OK)
                else:
                    return Response(replace_donation_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response("Creation Failed, Invalid donation_data !", status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response("Provide Required Data !", status=status.HTTP_400_BAD_REQUEST)
...
class DonationCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if {"donation_type", "donator_profile_data", "donation_data"} <= request.data.keys():
            # get request data
            request_data = request.data
            ...
            if request_data["donation_type"] == choices.INSIDE_DONATION:
                ...
                try:
                    donor_profile = DonorProfile.objects.get(pk=request_data["donator_profile_data"]["id"])
                    # mark the donator profile as old
                    donator_dic["donator_is_new"] = False
                except Exception:
                    donor_profile_serializer = DonorProfileSerializer(data=request_data["donator_profile_data"])
                    if donor_profile_serializer.is_valid():
                        ...
                        return Response(donor_profile_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
                ...
                for item in request_data["donation_data"]:
                    ...
                    if donation_serializer.is_valid():
                        ...
                        # if needed later
                        donator_dic["donator_items_ids"].append(donation_serializer.data["id"])
                        ...
                        if donator_dic["donator_is_new"] == True:
                            ...
                            donor_profile.personal_donation.filter(id__in=donator_dic["donator_items_ids"]).delete()
            ...
            # create outside donation
                ...
                # try to get the donator profile if it exist process with it or create a new one
                try:
                    receipt = Receipt.objects.get(receipt_number=request_data["donator_profile_data"]["id"])
                    # mark the donator profile as old
                    donator_dic["donator_is_new"] = False
                except Exception:
                    receipt_serializer = ReceiptSerializer(data=request_data["donator_profile_data"])
                ...
                for item in request_data["donation_data"]:
                    ...
                    if outside_donation_serializer.is_valid():
                        ...
                        # when the item is created append its id to institute_dict so i can delete it
                        # if needed later
                        donator_dic["donator_items_ids"].append(outside_donation_serializer.data["id"])
                        # append the serializer data to response data to return the items to frontend
                        ...
                        if donator_dic["donator_is_new"] == True:
                            ...
                            receipt.donated_units.filter(id__in=donator_dic["donator_items_ids"]).delete()
                            receipt.delete()
                            print("new deleted")
                            return Response(errors, status=status.HTTP_400_BAD_REQUEST)
                        else:
            ...
            # create replace donation
                ...
                try:
                    institute = InstituteProfile.objects.get(id=request_data["donator_profile_data"]["id"])
                    # mark the donator profile as old
                    donator_dic["donator_is_new"] = False
                except Exception:
                    institute_serializer = InstituteProfileSerializer(data=request_data["donator_profile_data"])
                ...
                for item in request_data["donation_data"]:
                    ...
                    if replace_donation_serializer.is_valid():
                        ...
                        # when the item is created append its id to institute_dict so i can delete it
                        # if needed later
                        donator_dic["donator_items_ids"].append(replace_donation_serializer.data["id"])
                        # append the serializer data to response data to return the items to frontend
                        ...
                        if donator_dic["donator_is_new"] == True:
                            ...
                            institute.replace_institute_donation.filter(id__in=donator_dic["donator_items_ids"]).delete()
                            institute.delete()
                            print("new deleted")
                            return Response(errors, status=status.HTTP_400_BAD_REQUEST)
                        else:
...
class DamageActionAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {"donation_type", "serial_number", "action_details"} <= request.data.keys():
            request_data = request.data
            obj_model = self.donation_type_manager[request_data["donation_type"]]["model"]
            obj_serializer = self.donation_type_manager[request_data["donation_type"]]["serializer"]

            try:
                obj = obj_model.objects.get(unit_serial_number=request_data["serial_number"])
            except obj_model.DoesNotExist:
            ...
            if request_data["action_details"]["action_type"] == "do_damage":
                ...
                return Response("Provide a Valid Action !", status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if {"search_in", "search_keyword"} <= request.data.keys():
            ...
            if request_data["search_in"] == "inside_donation":
                try:
                    obj = InsideDonation.objects.get(unit_serial_number=request_data["search_keyword"])
                except InsideDonation.DoesNotExist:
                    return Response("Donation Not Found. Note: Search Via Serial number", status=status.HTTP_400_BAD_REQUEST)
                ...
                try:
                    obj = OutsideDonation.objects.get(unit_serial_number=request_data["search_keyword"])
                ...
                try:
                    obj = ReplaceDonation.objects.get(unit_serial_number=request_data["search_keyword"])
                ...
                try:
                    ...
                    # but, it handeled
                    obj = DonorProfile.objects.get(national_id=int(request_data["search_keyword"]))
                    obj_serializer = DonorProfileSerializer(obj)
                except Exception:
                    obj = DonorProfile.objects.filter(full_name=request_data["search_keyword"])
                    obj_serializer = DonorProfileSerializer(obj, many=True)

                return Response(obj_serializer.data, status=status.HTTP_200_OK)

            elif request_data["search_in"] == "institute_profile":
                obj = InstituteProfile.objects.filter(full_name=request_data["search_keyword"])
                obj_serializer = InstituteProfileSerializer(obj, many=True)
                return Response(obj_serializer.data, status=status.HTTP_200_OK)
            else:
                return Response("Unsupported Search Type !", status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response("Please Provide Required Data !", status=status.HTTP_400_BAD_REQUEST)
        ...
        if {"donation_type", "unit_type_list", "is_separable"} <= request.data.keys():
            ...
            for key in request_data["donation_type"]:
                if not unit_type_list:
                    query = self.manage_queries["donation_type"][key]["model"].objects.filter(
